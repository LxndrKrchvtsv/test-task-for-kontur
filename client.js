...
const getEvents = async (formatDates = false) => {
    ...
    for (track in response) {
        ...
        let getDateEvent = new Date(date);
        getDateEvent = `${test.getDate()}-${test.getMonth() + 1}-${test.getFullYear()} ${test.getHours()}:${test.getMinutes()}:${test.getSeconds()}`
        result.events.push({
            "trackId": track,
            "event": event,
            "date": formatDates ? getDateEvent : date
        });
    }
    return result;
};
